...
namespace AoC_2020.Days
{
    ...
    {
        private Dictionary<string, (string color, int count)[]> contentsOf = new();
        private Dictionary<string, List<string>> containersOf = new();
        ...
        public override string Solve_2()
        {
            ...
            while (newcontent.Any())
            {
                ...
                var nextItems = newcontent.SelectMany(bag => contentsOf[bag.Key].Select(child => (child.color, count: bag.Value * child.count))).ToList();
                newcontent.Clear();
